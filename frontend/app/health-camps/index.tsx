...
export default function HealthCampsScreen() {
  ...
  const loadPrograms = async () => {
    try {
      ...
      // Combine API data with Mock data, avoiding duplicates if any
      const combined = [...apiPrograms, ...MOCK_HEALTH_CAMPS.filter(mc => !apiPrograms.some((ap: Program) => ap.id === mc.id))];
      setPrograms(combined);
    } catch (error) {
      console.error('Error loading programs:', error);
      // Fallback to mock data if API fails
      setPrograms(MOCK_HEALTH_CAMPS);
    } finally {
      setLoading(false);
    }
  };
